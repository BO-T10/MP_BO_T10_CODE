...
class Tracker(object):
    ...
    def step(self, output_results):
        ...
        track_bboxes = (
            output_results["track_boxes"] if "track_boxes" in output_results else None
        )  # x1y1x2y2
        ...
        for idx in range(scores.shape[0]):
            if idx in self.tracks_dict and track_bboxes is not None:
                self.tracks_dict[idx]["bbox"] = (
                    track_bboxes[idx, :].cpu().numpy().tolist()
                )
        ...
        if N > 0 and M > 0:
            det_box = torch.stack(
                [torch.tensor(obj["bbox"]) for obj in results], dim=0
            )  # N x 4
            track_box = torch.stack(
                [torch.tensor(obj["bbox"]) for obj in tracks], dim=0
            )  # M x 4
        ...
        if N_second > 0 and M_second > 0:
            det_box_second = torch.stack(
                [torch.tensor(obj["bbox"]) for obj in results_second], dim=0
            )  # N_second x 4
            track_box_second = torch.stack(
                [torch.tensor(obj["bbox"]) for obj in unmatched_tracks_obj], dim=0
            )  # M_second x 4
            cost_bbox_second = 1.0 - box_ops.generalized_box_iou(
                det_box_second, track_box_second
            )  # N_second x M_second

            matched_indices_second = linear_sum_assignment(cost_bbox_second)
            unmatched_tracks_second = [
                d for d in range(M_second) if not (d in matched_indices_second[1])
            ]
        ...
        self.tracks_dict = {
            red_ind: red
            for red_ind, red in results_dict.items()
            if "tracking_id" in red
        }
